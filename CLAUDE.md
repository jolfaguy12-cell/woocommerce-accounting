...
- **Frontend is mid-migration, off Inertia/React onto Blade + Alpine.js** (the pre-installed TailAdmin template, `resources/views/**`). Interactivity is plain GET/POST forms with full page reloads — no fetch/AJAX, Alpine only for cosmetic client-side bits (dropdowns, modals, column toggles). **Never build a new page in React** (`resources/js/pages/**`); it's being phased out page by page. Migrated to Blade: dashboard shell, orders, products, tools, settings, warehouse (packaging cost), notifications (notes), reports. Still Inertia/React (do not extend, only migrate): review, fast-forms, users. Staged plan: `/root/.claude/plans/glistening-giggling-kernighan.md`. Reports is fully off React/Inertia (2026-07-12) — Blade+Alpine is the only supported architecture for it; new report screens follow the `DashboardController`/`pages.dashboard.*` pattern, and ApexCharts (already wired via `resources/js/tailadmin/app.js`) is the official chart library for any future report charts.
