...
type ReportData = {
    ...
    data: {
        ...
        channels: Record<
            string,
            { name: string; orders: number; net_sales: number; operational_profit: number; period_cost: number; final_profitability: number }
        >;
    ...
    return (
        <AppLayout
            breadcrumbs={[
                ...
                { title: report.jalali_period, href: '#' },
            ]}
        >
            ...
            <div className="flex flex-col gap-4 p-4" dir="rtl">
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    {report.finalized_at && (
                        <span className="text-muted-foreground text-sm">نهایی‌شده: {new Date(report.finalized_at).toLocaleString('fa-IR')}</span>
                    )}
                </div>
                ...
                <div className="grid gap-4 lg:grid-cols-3">
                    <Card className="lg:col-span-2">
                        <CardHeader>
                            <CardTitle className="text-base">عملکرد سفارش‌ها</CardTitle>
                        </CardHeader>
                    ...
                    <Card>
                        <CardHeader>
                            <CardTitle className="text-base">جمع‌بندی دوره</CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-1 text-sm">
                            <div className="flex justify-between">
                                <span className="text-muted-foreground">سود عملیاتی سفارش‌ها</span>
                                <span dir="ltr">{fmt(d.orders.operational_profit)}</span>
                            </div>
                            <div className="flex justify-between">
                                <span className="text-muted-foreground">هزینه‌های مؤثر بر شرکا</span>
                                <span dir="ltr" className="text-red-600">
                                    -{fmt(d.expenses.total_affecting_partner)}
                                </span>
                            </div>
                            <div className="flex justify-between">
                                <span className="text-muted-foreground">حقوق دوره</span>
                                <span dir="ltr" className="text-red-600">
                                    -{fmt(d.payroll)}
                                </span>
                            </div>
                            {Object.entries(d.channel_costs).map(([slug, cost]) => (
                                <div key={slug} className="flex justify-between">
                                    <span className="text-muted-foreground">هزینه کانال {slug}</span>
                                    <span dir="ltr" className="text-red-600">
                                        -{fmt(cost)}
                                    </span>
                                </div>
                            ...
                            <div className="flex justify-between border-t pt-2 text-base font-bold">
                                ...
                                <span dir="ltr" className={d.net_period_profit < 0 ? 'text-red-600' : 'text-emerald-600'}>
                                    {fmt(d.net_period_profit)}
                                </span>
                            </div>
                        </CardContent>
                    </Card>
                ...
                <div className="grid gap-4 lg:grid-cols-2">
                    <Card>
                        <CardHeader>
                            <CardTitle className="text-base">کانال‌ها</CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-1 text-sm">
                            ...
                            {Object.entries(d.channels).map(([slug, ch]) => (
                                <div key={slug} className="flex justify-between border-b py-1 last:border-0">
                                    <span>
                                        {ch.name} ({fmt(ch.orders)})
                                    </span>
                                    <span dir="ltr" className={ch.final_profitability < 0 ? 'text-red-600' : ''}>
                    ...
                    <Card>
                        <CardHeader>
                            <CardTitle className="text-base">هزینه‌ها به تفکیک دسته</CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-1 text-sm">
                ...
                {report.adjustments.length > 0 && (
                    <Card>
                        <CardHeader>
                            <CardTitle className="text-base">تعدیلات پس از نهایی‌سازی</CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-1 text-sm">
                            {report.adjustments.map((a) => (
                                <div key={a.id} className="flex justify-between border-b py-1 last:border-0">
                                    ...
                                    <span className="text-muted-foreground" dir="ltr">
                                        {a.journal_entry?.jalali_period} · {a.journal_entry?.uuid.slice(0, 8)}
                                    </span>
                                </div>
                            ))}
                        </CardContent>
                    ...
                    <Card>
                        <CardContent className="flex flex-wrap items-center gap-3 py-3">
                            ...
                            <Button
                                variant="destructive"
                                onClick={() =>
                                    confirm('گزارش snapshot و دوره قفل می‌شود. ادامه؟') &&
                                    router.post(`/reports/${report.jalali_period}/finalize`, { acknowledge }, { preserveScroll: true })
                                }
                            >
