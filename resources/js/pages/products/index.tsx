...
type Row = {
    id: number; hub_product_id: number; name: string; type: string; sku: string | null;
    price: number | null; stock_quantity: number | null; mapping_status: string;
};
            ...
            <div className="flex flex-col gap-4 p-4" dir="rtl">
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    <Input
                        ...
                        onKeyDown={(e) => e.key === 'Enter' && router.get('/products', { ...filters, q: e.currentTarget.value }, { preserveState: true })}
                    />
                    <label className="flex items-center gap-1 text-sm">
                        <input
                            type="checkbox"
                            defaultChecked={filters.mapping === 'unmapped'}
                            onChange={(e) => router.get('/products', { ...filters, mapping: e.target.checked ? 'unmapped' : undefined }, { preserveState: true })}
                        />
                        فقط بدون نگاشت
                    </label>
                </div>
                        ...
                        <table className="w-full text-sm">
                            ...
                            <tbody>
                                {products.data.map((p) => (
                                    <tr key={p.id} className="border-b last:border-0 hover:bg-muted/40">
                                        <td className="p-3">
                                            ...
                                            <span className="mr-2 text-xs text-muted-foreground" dir="ltr">#{p.hub_product_id}</span>
                                        </td>
                                        ...
                                        <td>
                                            <Badge variant={p.mapping_status === 'mapped' ? 'default' : p.type === 'variable' ? 'outline' : 'destructive'}>
                                ...
                                {products.data.length === 0 && (
                                    <tr><td colSpan={6} className="p-6 text-center text-muted-foreground">محصولی یافت نشد — با acc:sync:product همگام‌سازی کنید</td></tr>
                                )}
                            </tbody>
                        </table>
                    </CardContent>
