...
function MapSourceForm({ source, channels }: { source: NonNullable<Item['source']>; channels: Channel[] }) {
    ...
    const submit = () => {
        router.post(`/review/sources/${source.id}/map`, channelId ? { channel_id: Number(channelId) } : { new_channel_name: newName, new_channel_cost_model: 'none' }, { preserveScroll: true });
    };

    return (
        <div className="flex flex-wrap items-center gap-2">
            <select className="h-9 rounded-md border bg-background px-2 text-sm" value={channelId} onChange={(e) => setChannelId(e.target.value)}>
            ...
            <Button size="sm" onClick={submit} disabled={!channelId && newName.trim() === ''}>اتصال</Button>
        </div>
    );
}

export default function Review({ items, channels }: { items: Item[]; channels: Channel[] }) {
    const close = (item: Item, action: 'resolved' | 'dismissed') =>
        router.post(`/review/${item.id}/resolve`, { action }, { preserveScroll: true });
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex flex-col gap-4 p-4" dir="rtl">
                ...
                {items.length === 0 && (
                    <Card><CardContent className="py-8 text-center text-muted-foreground">همه‌چیز پاک است ✅</CardContent></Card>
                )}
                {items.map((item) => (
                    <Card key={item.id}>
                        <CardHeader className="pb-2">
                            <CardTitle className="flex items-center gap-2 text-base">
                                <Badge variant={item.type === 'sync_error' ? 'destructive' : 'secondary'}>
                        ...
                        <CardContent className="flex flex-wrap items-center justify-between gap-3">
                            <div className="text-sm">
                                {item.source ? (
                                    <span>
                                        منبع خام: <code className="rounded bg-muted px-1">{item.source.raw_value}</code>{' '}
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <Button size="sm" variant="outline" onClick={() => close(item, 'resolved')}>حل شد</Button>
                                <Button size="sm" variant="ghost" onClick={() => close(item, 'dismissed')}>نادیده</Button>
                            </div>
                        </CardContent>
                    </Card>
                ))}
