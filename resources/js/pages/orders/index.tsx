...
type Row = {
    ...
    jalali_period: string; channel: string | null; total: number; operational_profit: number | null; order_date: string;
};
            ...
            <div className="flex flex-col gap-4 p-4" dir="rtl">
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    <Input
                        ...
                        onKeyDown={(e) => e.key === 'Enter' && router.get('/orders', { ...filters, period: e.currentTarget.value }, { preserveState: true })}
                    />
                    <select
                            ...
                            <tbody>
                                {orders.data.map((o) => (
                                    <tr key={o.id} className="border-b last:border-0 hover:bg-muted/40">
                                        ...
                                        <td><Badge variant={statusVariant(o.profit_status)}>{o.profit_status}</Badge></td>
                                    </tr>
                                ))}
                                {orders.data.length === 0 && (
                                    <tr><td colSpan={7} className="p-6 text-center text-muted-foreground">سفارشی یافت نشد</td></tr>
                                )}
                            </tbody>
                        </table>
                    </CardContent>
