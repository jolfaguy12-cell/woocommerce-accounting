{{--
    Shared shell for the auth screens (login, forgot/reset/confirm password,
    verify email). Split layout: form on one side, branding on the other.
    Pages fill @section('auth-title'), @section('auth-description') and
    @section('auth-form').
--}}
        ...
        <div class="relative flex h-screen w-full flex-col justify-center sm:p-0 lg:flex-row dark:bg-gray-900">
            ...
            <div class="flex w-full flex-1 flex-col lg:w-1/2">
                <div class="mx-auto flex w-full max-w-md flex-1 flex-col justify-center">
                    ...
                    @if (session('status'))
                        <div class="mb-4 rounded-lg border border-success-500/30 bg-success-50 px-4 py-2.5 text-sm text-success-600 dark:bg-success-500/10 dark:text-success-500">
                            {{ session('status') }}
                        </div>
                    @endif
            ...
            <div class="relative hidden h-full w-full items-center bg-brand-950 lg:grid lg:w-1/2 dark:bg-white/5">
                <div class="z-1 flex items-center justify-center">
                    ...
                    <div class="flex max-w-xs flex-col items-center">
                        <a href="/" class="mb-4 block text-2xl font-bold text-white">
                            {{ config('app.name') }}
                        </a>
                        <p class="text-center text-gray-400 dark:text-white/60">
                            سامانه داخلی حسابداری و گزارش‌گیری
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="fixed bottom-6 left-6 z-50">
                <button type="button" @click.prevent="$store.theme.toggle()"
