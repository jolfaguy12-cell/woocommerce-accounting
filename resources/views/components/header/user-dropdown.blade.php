<div class="relative" x-data="{
    ...
    <button
        ...
        type="button"
    >
        <span class="overflow-hidden rounded-full h-8 w-8 shrink-0">
            <img src="/images/user/owner.png" alt="User" />
        </span>

        <span x-show="$store.sidebar.isExpanded || $store.sidebar.isHovered || $store.sidebar.isMobileOpen"
            class="menu-item-text flex items-center flex-1 gap-2 overflow-hidden">
            <span class="truncate">{{ auth()->user()->name }}</span>

            <!-- Chevron Icon -->
            <svg
                class="w-5 h-5 mr-auto transition-transform duration-200 shrink-0"
    ...
    <!-- Dropdown Start (opens upward — anchored at the bottom of the sidebar) -->
    <div
        x-show="dropdownOpen"
        ...
        class="absolute left-0 bottom-full mb-[17px] flex w-[260px] flex-col rounded-2xl border border-gray-200 bg-white p-3 shadow-theme-lg dark:border-gray-800 dark:bg-gray-dark z-50"
        style="display: none;"
    >
        <!-- User Info -->
        <div>
            ...
            @endphp
            @if ($roleName)
                <span class="mt-0.5 block text-theme-xs text-gray-500 dark:text-gray-400">{{ $roleLabels[$roleName] ?? $roleName }}</span>
            @endif
        </div>
