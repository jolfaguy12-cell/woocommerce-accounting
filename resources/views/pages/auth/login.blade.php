...
@section('auth-title', 'ورود به سامانه')
@section('auth-description', 'برای ورود، ایمیل و رمز عبور خود را وارد کنید')
...
@section('auth-form')
    <form method="POST" action="{{ route('login') }}" class="space-y-5">
        ...
        <div>
            ...
            <div x-data="{ show: false }" class="relative">
                <input :type="show ? 'text' : 'password'" id="password" name="password" required autocomplete="current-password"
                    dir="ltr" placeholder="********"
                    class="h-11 w-full rounded-lg border border-gray-300 bg-transparent py-2.5 pl-11 pr-4 text-left text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">
                <span @click="show = !show" class="absolute left-4 top-1/2 z-30 -translate-y-1/2 cursor-pointer text-gray-500 dark:text-gray-400">
        ...
        <p class="text-center text-sm text-gray-500 dark:text-gray-400">
            حساب کاربری توسط مدیر سامانه ساخته می‌شود.
        </p>
    </form>
@endsection
