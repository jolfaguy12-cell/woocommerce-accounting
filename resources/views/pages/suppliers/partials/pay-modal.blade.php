...
<div x-data="{ open: false }" @open-pay-supplier-modal.window="open = true">
    <x-ui.modal :isOpen="$errors->has('amount') && old('_form') === 'pay'" @open-pay-supplier-modal.window="open = true" class="max-w-sm p-6">
        ...
        <form method="POST" action="{{ route('suppliers.pay', $supplier) }}" class="space-y-4">
            ...
            @include('pages.suppliers.partials.payment-fields', ['bankAccounts' => $bankAccounts, 'bankLabel' => 'پرداخت از حساب'])
