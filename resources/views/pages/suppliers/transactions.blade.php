        ...
        >
            @foreach ($transactions as $line)
                ...
                <tr class="border-b border-gray-100 last:border-0 dark:border-gray-800">
                    <td x-show="visible.date" class="whitespace-nowrap px-5 py-3 text-xs text-gray-500 sm:px-6 dark:text-gray-400">{{ $row['date'] }}</td>
                    ...
                    <td x-show="visible.description" class="px-5 py-3 text-gray-800 sm:px-6 dark:text-white/90">{{ $row['description'] }}</td>
                    ...
                    <td x-show="visible.method" class="px-5 py-3 text-gray-600 sm:px-6 dark:text-gray-300">
                        {{ $payment ? ($methodLabels[$payment->method] ?? '—') : '—' }}
                    </td>
                    <x-tables.ltr x-show="visible.reference" class="px-5 sm:px-6" :value="$payment->reference ?? null" tone="muted" />
