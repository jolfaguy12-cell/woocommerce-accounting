...
@php
    // Same pro-table composition as pages.customers.index, pre-filtered to
    // is_wholesale=true — no wholesale column/filter here since every row
    // already qualifies; a telegram column replaces it instead.
    $columns = [
    ...
    $filterLabels = ['channel_id' => 'کانال'];
@endphp

@section('content')
<x-common.page-breadcrumb pageTitle="مشتریان عمده" />
...
<x-tables.pro-table
    ...
    empty-message="هنوز مشتری عمده‌ای علامت‌گذاری نشده است"
    search-value="{{ $filters['search'] ?? '' }}"
    search-placeholder="جستجوی نام، شماره تماس یا آیدی تلگرام"
    :clear-filters-route="$query->hasActiveFilters() ? $query->clearUrl() : null"
    storage-key="wholesale-customers.visibleColumns"
>
    <x-slot:filters>
    ...
    @foreach ($customers as $customer)
        <tr class="border-b border-gray-100 last:border-0 dark:border-gray-800">
            ...
            <td x-show="visible.telegram" class="px-5 py-3 sm:px-6">
                @if ($customer->telegram_id)
                    <x-tables.ltr :value="$customer->telegram_id" :cell="false" tone="muted" />
                @else
                    <a href="{{ route('customers.show', $customer) }}" class="text-xs text-brand-500 hover:underline">بدون آیدی — ثبت آیدی</a>
                @endif
            </td>
